  ...
  const [filtroOrdenacao, setFiltroOrdenacao] =
    ...
    useEffect(() => {
      const fetchJogadores = async () => {
        try {
          const docRef = doc(db, "combine", "jogadores");
          const docSnap = await getDoc(docRef);
    
          if (!docSnap.exists()) {
            console.log("Documento 'jogadores' não encontrado");
            return;
          }
    
          const jogadoresData = docSnap.data(); // isso será um objeto com vários jogadores
          const jogadoresList: Jogador[] = [];
    
          for (const nome in jogadoresData) {
            const jogadorData = jogadoresData[nome];
          ...
          console.error("Erro ao carregar os jogadores do combine:", error);
        }
      };
